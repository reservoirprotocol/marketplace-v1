...
const MyDocument = function () {
  ...
  return (
    <Html className={DARK_MODE ? 'dark' : ''}>
      ...
      <meta property='og:image:alt' content={`${SOURCE_NAME || SOURCE_ID || 'Market'} banner`} />

      {/* Reservoir Meta Tags */}
      {SOURCE_NAME ? <meta property='reservoir:title' content={SOURCE_NAME} /> : null}
      {SOURCE_ICON || FAVICON ? <meta property='reservoir:icon' content={SOURCE_ICON || FAVICON} /> : null}
      ...
      {FONT_URLS ? FONT_URLS.split(',').map((link, i) => <link key={i} href={link} rel='stylesheet' />) : null}
